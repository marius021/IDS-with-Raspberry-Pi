...
try:
    ...
    raise SystemExit(
        "Nu am putut importa binding-urile Hailo Python (`hailo_platform`).\n"
        f"Eroare: {e}\n"
        "Verifică pe Raspberry că ai instalat HailoRT Python bindings."
    )
...
class HailoRunner:
    def __init__(self, hef_path: Path):
        ...
        if not self.input_infos:
            raise RuntimeError("HEF-ul nu are input vstreams.")
...
def main():
    ...
    print(
        f"[INFO] Pornit HAILO IPS. Monitorizez: {input_csv} | poll={args.poll}s | "
